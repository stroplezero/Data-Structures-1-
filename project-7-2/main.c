...
int main() {
	...
	FILE *fp = fopen("C:\\Users\\a0103\\Desktop\\name_list.txt", "r");
	if (fp == NULL) {
		printf("Fail to open\n");
		free(list);
		return 0;
	...
	struct student max = *list;
	struct student min = *list;
	int sum = list->grade;

	for (int i = 1; i < SIZE; i++) {
		struct student *current = (list + i); 
		if (max.grade < current->grade) {
			max = *current;
		}
		if (min.grade > current->grade) {
			min = *current;
		}
