...
## 2. GCD와 피보나치의 시간 복잡도 분석
- 주요 알고리즘: 재귀적 방법(피보나치 수열) 
- 설명: 입력값 n에 대해 n번째 피보나치 수를 구하는 프로그램 F(n)을 재귀적 방법으로 구현하고, 연속하는 피보나치의 두 수를 구하여 GCD를 구하는 알고리즘을 작성하면, 가장 중요한 코드는 F(n)을 구하기 위해 반복해서 연산하는 피보나치 수를 구하는 코드이다.  
이 코드는 n이 1 증가할 때마다 자기 자신의 코드를 2번씩 추가로 더 호출하므로 2^n번 실행된다고 할 수 있다.  

 따라서 GCD와 피보나치의 시간 복잡도는 O(2^n)의 시간 복잡도를 가진다고 할 수 있다.  
...
## 4. GCD와 피보나치의 시간 복잡도 검증
 실행 시간이 유사한 30번부터 34번까지의 결과(4초 대)는 제외하고 34번부터 50번까지의 결과를 살펴보았다.  
...
| 48-> 50 | 799 -> 2110.667 | +1311.667 | 약 2.64배 |
 ...
 따라서 40회 이후의 시간 증가량을 계산하면 보다 정확한 GCD와 피보나치의 시간 복잡도를 증명할 수 있다.

- 40회 이후의 시간 증가량 계산
 40회부터 이후의 시간 증가량의 평균을 계산해보면 약 2.496배로 1회가 늘어날 때마다 약 1.248배 늘어남을 알 수 있다.
  
 따라서 GCD와 피보나치 수열의 시간 복잡도를 구하면 O(2^n)임을 알 수 있으며, 위에서 추론한 시간 복잡도와 일치함을 알 수 있다. 
