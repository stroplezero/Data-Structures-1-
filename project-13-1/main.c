	...
	while (check) {
		...
		if (menuNum < 0 || menuNum > 6) {
			...
			case(1):
				...
				scanf("%d", &newNum);
				while (getchar() != '\n');
				insertFirstLinkedList(myLinkedList, newNum);
			...
			case(2):
				...
				scanf("%d", &newNum);
				while (getchar() != '\n');
				insertLastLinkedList(myLinkedList, newNum);
			...
			case(3):
				...
				while (getchar() != '\n');
				insertAtlinkedList(myLinkedList, pos, newNum);
			...
			case(4):
				...
				scanf("%d", &pos);
				while (getchar() != '\n');
				deleteAtLinkedList(myLinkedList, pos);
			...
			case(6):
				printf("6. [리스트 크기 확인]\n");
				printf("size: %d\n", sizeLinkedList(myLinkedList));

				break;

			case(0):
				printf("0. [종료] ");
				int i = destroyLinkedList(myLinkedList);
